...
func formatte(date: String) -> Date {
...
let newsModel: [NewsModel] = models.map { element in
    NewsModel(title: element.title, publishedAt: formatte(date: element.publishedAt))
}
var newDate = {(date: Date) -> Date in
...
let newsDictionary = Dictionary(grouping: newsModel) { element in
    newDate(element.publishedAt)
}
...
let news: [News] = newsDictionary.map { (key: Date, value: [NewsModel]) in
    News(nameSection: key, cell: value)
}.sorted { News1, News2 in
    News1.nameSection > News2.nameSection
}
